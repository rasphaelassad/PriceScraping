...
class PriceService:
    ...
    async def get_prices(self, request: PriceRequest) -> Dict[str, UrlResult]:
        ...
        try:
            ...
            # Start background processing for URLs that need it
            urls_to_process = [
                url for url, result in final_results.items()
                if result.request_status.status in ['pending', 'running']
            ]
            if urls_to_process:
                task = asyncio.create_task(
                    self._process_urls_background(store_name, urls_to_process)
                )
            ...
            for url in urls:
                try:
                    if url in results:
                        ...
                        self.cache_manager.update_cache_with_result(url, store_name, results[url])
                    else:
                        # Update cache with error if URL not in results
                        error_msg = "URL not found in scraper results"
                        self.cache_manager.update_cache_with_error(url, store_name, error_msg)
                except Exception as e:
                    logger.error(f"Error updating cache for {url}: {e}")
                    self.cache_manager.update_cache_with_error(url, store_name, str(e))
